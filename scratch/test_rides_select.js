...
async function main() {
  const { data, error } = await supabase
    .from('rides')
    .select(
      'id, status, fare, ride_type, vehicle_type, pickup_label, destination_label, distance_km, duration_min, scheduled_at, completed_at, created_at'
    )
    .limit(1);
  ...
  if (error) {
    ...
    console.log('Query Success! Columns are correct. Data:', data);
  }
}
